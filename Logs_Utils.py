...
def merge_logs_to_games(
    ...
    if not overwrite:
        log_start = get_end(Utils.openfile(sorted(output_folder.glob("*.json"))[-1]))
...
def get_files_after_date(folder: str | Path, date: str) -> list:
    def get_date(file: Path) -> datetime:
        _datetime = datetime.fromisoformat(Utils.recuperate_date(file.stem))
