...
def plot_AKPM_KPM_DPM(
    player_id: str, folder_plots: Path | str, list_players_names: dict
):
    years = ["2022", "2023", "2024", "2025"]
    player_years = [
        grab_player_plot(
            player_id, folder_plots, ["list Apolo kpm", "list kpm", "list dpm"], year
        )
        for year in years
    ]
    ...
    for player_year in player_years:
        for key, sub_dict in player_year.items():
            merged[key] = deep_merge(merged[key], sub_dict)

    player_plots = {}
    for plot in ["list Apolo kpm", "list kpm", "list dpm"]:
        player_akpm = pd.Series(
            {
                Utils.recuperate_date(x): value
                for x, value in merged[player_id][plot].items()
            }
        )
        player_akpm.index = pd.to_datetime(player_akpm.index)
        player_plots[plot] = player_akpm.resample("M").mean()
    df = pd.DataFrame(
        {
            "AKPM": player_plots["list Apolo kpm"],
            "KPM": player_plots["list kpm"],
            "DPM": player_plots["list dpm"],
        }
    )
    who = list_players_names[player_id][0]
    ax = df.plot()
    plt.style.use("fivethirtyeight")
    # hide the splines
    for side in ["left", "right", "top"]:
        ax.spines[side].set_visible(False)
    ax.yaxis.grid(True, linestyle="-", linewidth=0.7, alpha=0.7)  # Solid grid lines

    # Format the x-axis to use MonthLocator and DateFormatter
    # ax.xaxis.set_major_locator(mdates.MonthLocator())
    # ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))

    ax.tick_params(axis="y", length=0, labelcolor="grey")
    ax.tick_params(axis="x", length=0, labelcolor="grey")
    ...
    plt.ylabel("AKPM", color="grey")
    plt.title(f"{who} AKPM per month", color="grey")
    plt.ylim(0, 1.5)
    plt.tight_layout()
    plt.savefig(f"{who}_akpm_plot.png", dpi=300, bbox_inches="tight")
...
def grab_player_plot(
    ...
    """
    Grab data from a player in a folder and its subfolders.
    ...
    plots : str | list
        The plots to grab
    month : str
        The month to grab (optional)
    day : str
        The day to grab (optional)
    ...
    """
    folder = Path(folder)
    files = Utils.grab_games_by_dates(folder, month, day, separator="_")
    result = {}
    for file in files:
...
def player_total_seconds(game: dict, player_id: str) -> int:
    ...
    """
    start, end = Utils.start_end_isostring(game)
    game = Utils.only_actual_game_logs(game)
    player_connections = [
        (x["type"], x["event_time"])
        for x in game["logs"]
        if (
...
def refill_analysis_folder(out_folder_analysis: str | Path, folder_games: str | Path):
    ...
    for file in folder_games.glob("*.json"):
        if file.name > last_game_file_analysed:
            ...
            game = Utils.openfile(file)
            if not Logs_Utils.check_game(game):
